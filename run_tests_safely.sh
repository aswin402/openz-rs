...
echo "Restricting compilation to 2 parallel jobs (-j 2)"
echo "Running tests sequentially (--test-threads=1)"
echo "=========================================="

# 1. compile first with restricted cores
cargo test --offline --no-run -j 2

# 2. Run test modules sequentially
echo ""
echo "👉 Batch 1: Self-Management & Configuration Tools"
cargo test --offline -j 2 tools::self_management -- --test-threads=1

echo ""
echo "👉 Batch 2: Headroom (Context Compression) Tools"
cargo test --offline -j 2 tools::headroom -- --test-threads=1

echo ""
echo "👉 Batch 3: Extended & Shared Memory Systems"
cargo test --offline -j 2 tools::memory_extra -- --test-threads=1
cargo test --offline -j 2 tools::shared_memory -- --test-threads=1

echo ""
echo "👉 Batch 4: Sequential Thinking & Reasonings"
cargo test --offline -j 2 tools::sequential_thinking -- --test-threads=1

echo ""
echo "👉 Batch 5: Database & File System Tools"
cargo test --offline -j 2 tools::db_inspector -- --test-threads=1
cargo test --offline -j 2 tools::filesystem -- --test-threads=1

echo ""
echo "👉 Batch 6: SearchXyz & Web Crawlers"
cargo test --offline -j 2 tools::searchxyz -- --test-threads=1
cargo test --offline -j 2 tools::web -- --test-threads=1
cargo test --offline -j 2 tools::web_search -- --test-threads=1
...
cargo test --offline -j 2 -- --test-threads=1

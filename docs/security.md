...
In addition to command and path inspection, the shared resource policy requires approval for tools that provide no recognizable metadata signal (`general` domain with no network, process, or disk capability). This keeps unknown integrations conservative without adding a second prompt for tools already covered by the SecurityGuard or resource limits.

---

## 🚫 Intercepted Tool Categories

The guard matches commands using whole-token word detection to prevent bypasses and false positives. High-risk tools also inherit an approval prompt from shared `ToolRisk` metadata; command and path inspection remains argument-sensitive. The following categories of actions will trigger an approval prompt:
...
### 6. High-Risk File Operations and Out-of-Workspace Writing
* Native `write_file`, `patch_file`, and `replace_lines` calls require approval through shared high-risk metadata.
* Calls targeted outside the configured `/workspace` directory or the user's active `/home/` directory (e.g. attempting to edit files in `/etc/`, `/usr/bin/`, or `/var/`) are additionally caught by the path guard.

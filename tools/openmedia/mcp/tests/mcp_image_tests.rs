use openmedia_mcp::{
    OpenMediaServer, Parameters, ImageApplyFilterRequest, ImageResizeRequest,
    ImageCropRequest, ImageTransformRequest, ImageConvertRequest, ImageBatchProcessRequest,
};
...
async fn test_mcp_image_processing_tools() {
    ...
    let img = image::DynamicImage::ImageRgba8(image::RgbaImage::from_pixel(32, 32, image::Rgba([100, 150, 200, 255])));
    ...
    let batch_params = Parameters(ImageBatchProcessRequest {
        ...
        operations: vec![serde_json::to_value(openmedia_process::ProcessOperation::Invert).unwrap()],

...
async fn ensure_geckodriver_running(port: u16, connect_existing: bool) -> Result<()> {
    ...
    if let Ok(child_handle) = Command::new("geckodriver")
        .args({ let mut args = vec!["--port".to_string(), port.to_string()]; if connect_existing { args.push("--connect-existing".to_string()); } args })
        ...
        .spawn()
    {
        crate::shutdown::register_child_with_metadata(
            child_handle,
            if connect_existing { "geckodriver --connect-existing" } else { "geckodriver --port 4444" },
            "geckodriver",
        );
        for _ in 0..15 {
...
async fn get_or_create_driver(mode: &str) -> Result<WebDriver> {
    ...
    let port = if connect_existing { 4445 } else { 4444 };
...
impl Tool for FirefoxBrowserTool {
    ...
    async fn call(&self, arguments: &Value) -> Result<Value> {
        ...
        if !matches!(mode, "headless" | "visible" | "attach") {
            return Err(anyhow!("Unsupported Firefox mode {}; use headless, visible, or attach", mode));
        }
        ...
        let result = match action {
            ...
            "screenshot" => {
                ...
                let device_inventory_recorded = if opened {
                    ...
                    crate::tools::device_inventory::record_successful_default_open(path).ok().flatten()
                } else {
                    None
                };
                json!({

...
fn parse_effect_cache_key_is_order_insensitive_for_object_params() {
    ...
    let a = crate::tools::wavyte::eval::ResolvedEffect {
        kind: "blur".to_string(),
        params: serde_json::json!({ "radius_px": 3, "sigma": 2.0 }),
    };
    let b = crate::tools::wavyte::eval::ResolvedEffect {
        kind: "blur".to_string(),
...
fn parse_transition_cache_key_is_order_insensitive_for_object_params() {
    let mut cache = CompileCache::default();
    let a = crate::tools::wavyte::eval::ResolvedTransition {
        kind: "wipe".to_string(),
        progress: 0.5,
        params: serde_json::json!({ "dir": "ttb", "soft_edge": 0.2 }),
    };
    let b = crate::tools::wavyte::eval::ResolvedTransition {
        kind: "wipe".to_string(),
        progress: 0.5,
        ...
        tracks: vec![Track {
            ...
            clips: vec![Clip {
                ...
                transition_in: Some(TransitionSpec {
                    kind: "fade".to_string(),
                    duration_frames: 2,
                    ease: crate::tools::wavyte::Ease::Linear,
...
fn compile_emits_offscreen_blur_pass_and_composites_blurred_surface() {
    ...
    match &plan.passes[1] {
        Pass::Offscreen(p) => {
            ...
            assert_eq!(
                p.fx,
                crate::tools::wavyte::effects::fx::PassFx::Blur {
                    radius_px: 3,
                    sigma: 2.0
                }
            );

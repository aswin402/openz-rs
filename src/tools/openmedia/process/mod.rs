...
impl FilterChain {
    ...
    pub fn apply(&self, img: &image::DynamicImage) -> crate::tools::openmedia::core::Result<image::DynamicImage> {
        let mut current_img = img.clone();
        for op in &self.operations {
            // Prefer GPU, fallback to CPU
            current_img = if let Ok(gpu_img) = self::gpu::apply_gpu_operation(&current_img, op) {
                gpu_img
            } else {
                self::cpu::apply_cpu_operation(&current_img, op)?
            };
        }
        Ok(current_img)
    }
...
pub async fn batch_process_files(
...
) -> crate::tools::openmedia::core::Result<Vec<std::path::PathBuf>> {
    let paths_iter = glob::glob(glob_pattern).map_err(|e| {
        crate::tools::openmedia::core::OpenMediaError::IoError(std::io::Error::new(
    ...
    for path in paths {
        ...
        tasks.push(tokio::task::spawn_blocking(
            move || -> crate::tools::openmedia::core::Result<std::path::PathBuf> {
                ...
                processed.save(&dest).map_err(|e| {
                    crate::tools::openmedia::core::OpenMediaError::ImageEncodeError {
                        format: dest
                            .extension()
    ...
    for task in tasks {
        let path = task
            .await
            .map_err(|e| crate::tools::openmedia::core::OpenMediaError::Internal(e.to_string()))??;

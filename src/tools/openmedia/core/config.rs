use super::error::{OpenMediaError, Result};
...
impl Config {
    ...
    pub fn load() -> Result<Self> {
        ...
        let config_path = openz_dir.join("media/config.toml");
        let legacy_config_path = home.join(".openmedia/config.toml");

        let target_config = if config_path.exists() {
            Some(config_path)
...
impl Default for Config {
    fn default() -> Self {
        ...
        let openz_dir = std::env::var("OPENZ_CONFIG_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| home.join(".openz"));
        let media_dir = openz_dir.join("media");
        ...
        Self {
            ...
            paths: PathConfig {
                model_dir: media_dir.join("models"),
                output_dir: media_dir.join("output"),
                history_db: media_dir.join("history.db"),
                image_subdir: "images".into(),
                video_subdir: "videos".into(),
                svg_subdir: "svgs".into(),
                checksum_file: media_dir.join("models/checksums.sha256"),
            },

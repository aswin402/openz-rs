...
use ndarray::Array;
use crate::tools::openmedia::core::{OpenMediaError, Result};
    ...
    pub async fn score(&self, image_path: &Path, prompt: &str) -> Result<f32> {
        ...
        let img = image::open(image_path).map_err(|e| {
            OpenMediaError::ImageDecodeError(format!(
                "Failed to open image for CLIP scoring: {}",
                e
            ))
        })?;
        ...
        let mask_array =
            Array::from_shape_vec(ndarray::IxDyn(&[1, 77]), attention_mask).map_err(|e| {
                OpenMediaError::Internal(format!("Failed to build attention mask array: {}", e))
            })?;
        let mask_tensor = Value::from_array(mask_array).map_err(|e| {
            OpenMediaError::Internal(format!("Failed to build attention mask tensor: {}", e))
        })?;

        // 3. Run vision model
        let vision_inputs = vec![("pixel_values", image_tensor)];
        let vision_outputs = vision_guard
            .run(vision_inputs)
            .map_err(|e| OpenMediaError::Internal(format!("Failed to run vision model: {}", e)))?;

        let img_embeds_value = vision_outputs
            .iter()
            .next()
            .map(|(_, v)| v)
            .ok_or_else(|| OpenMediaError::Internal("Vision model output not found".into()))?;

        let img_embeds_view = img_embeds_value.try_extract_tensor::<f32>().map_err(|e| {
            OpenMediaError::Internal(format!("Failed to extract vision embedding: {}", e))
        })?;

        let img_embeds = img_embeds_view.1;

        // 4. Run text model
        let text_inputs = vec![("input_ids", text_tensor), ("attention_mask", mask_tensor)];
        let text_outputs = text_guard
            .run(text_inputs)
            .map_err(|e| OpenMediaError::Internal(format!("Failed to run text model: {}", e)))?;

        let text_embeds_value = text_outputs
            .iter()
            .next()
            .map(|(_, v)| v)
            .ok_or_else(|| OpenMediaError::Internal("Text model output not found".into()))?;

        let text_embeds_view = text_embeds_value.try_extract_tensor::<f32>().map_err(|e| {
            OpenMediaError::Internal(format!("Failed to extract text embedding: {}", e))
        })?;
    ...
    pub async fn load(model_path: &Path) -> Result<Self> {
        if model_path.exists() {
            match (|| -> std::result::Result<Session, ort::Error> {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Failed to load Aesthetic model from {:?}: {}",
                        model_path,
                        e
                    );
                }
            ...
            tracing::info!(
                "Aesthetic predictor ONNX model not found at {:?}",
                model_path
            );
        }

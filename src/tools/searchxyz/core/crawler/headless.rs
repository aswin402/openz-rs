use crate::tools::searchxyz::core::config::HeadlessConfig;
...
pub struct HeadlessBrowser {
    ...
    #[allow(dead_code)]
    proxy_config: crate::tools::searchxyz::core::config::ProxyConfig,
}

impl HeadlessBrowser {
    pub fn new(config: HeadlessConfig, proxy_config: crate::tools::searchxyz::core::config::ProxyConfig) -> Self {
        Self {
            config,
            proxy_config,
        }
    ...
    pub async fn fetch_html(&self, url: &str) -> Result<String, SearchXyzError> {
        ...
        if self.proxy_config.enabled && !self.proxy_config.urls.is_empty() {
            ...
            if let Some(proxy_url) = self.proxy_config.urls.choose(&mut rand::thread_rng()) {
        ...
        let rand_headers = crate::tools::searchxyz::core::crawler::fingerprint::HeaderGenerator::random_headers();

...
mod tests {
    ...
    async fn test_static_semantic_fact_store() {
        ...
        working::store_semantic_fact("test-fact-1", "Rust is a systems language.", 0.8, uid, sid, aid).unwrap();

        let results = with_db(|conn| search::query_fts5(conn, "systems language", 10, uid, sid, aid)).unwrap();
    ...
    async fn test_search_text_fts5() {
        ...
        working::store_semantic_fact("fts-fact-1", "MCP defines a standard protocol for context-aware AI tools.", 0.9, uid, sid, aid).unwrap();
        working::store_semantic_fact("fts-fact-2", "SQLite is a self-contained SQL database engine.", 0.7, uid, sid, aid).unwrap();

        let results = with_db(|conn| search::query_fts5(conn, "context-aware", 10, uid, sid, aid)).unwrap();
    ...
    async fn test_proactive_recall() {
        ...
        working::store_semantic_fact("pr-fact-1", "Rust compiler optimizations improve performance.", 0.9, uid, sid, aid).unwrap();
